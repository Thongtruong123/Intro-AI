...
from algorithm import astar_path, bfs_path, dfs_path, dijkstra_path, nearest_node, bellman_ford, heuristic
    ...
    def calculate_route(self):
        ...
        if self.selected_algorithm == "DFS" or self.selected_algorithm == "BFS":
            ...
            if self.selected_algorithm == "DFS":
                ...
                path, _ = bfs_path(graph, self.start_node, self.end_node)
            if path is None:
            ...
            if self.selected_algorithm == "A*":
                for id1, i in enumerate(near_start):
                    for id2, j in enumerate(near_end):
                        path, dis = astar_path(graph, i, j, heuristic= astar_path.heuristic)
                        if path is None:
                            continue
                ...
                for id1, i in enumerate(near_start):
                    for id2, j in enumerate(near_end):
                        path, dis = dijkstra_path(graph, i, j)
                        if path is None:
                            continue
                        dis += (distance_start[id1] + distance_end[id2])
                        if dis < dis_min:
